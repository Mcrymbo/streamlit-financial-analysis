...
if shap_available:
    sv, Xs = _get_shap(best_model, X_sample, DISP_FEATURES)
    ...
    shap_lib.summary_plot(sv, Xs, feature_names=DISP_FEATURES,
    ...
    st.caption("Importance measured as mean decrease in R² when each feature is randomly shuffled.")
...
# Get top 4 features by permutation importance
...
for ax, fi_idx in zip(axes, top4_idx):
    ...
    ax.axvline(feat_vals.mean(), color="#FDD835", lw=1.2, ls="--", alpha=0.7,
               label="mean")
